...
export const inputClass = 'block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm outline-none transition-colors focus:border-brand-600 focus:ring-2 focus:ring-brand-600/20 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100'
...
interface SectionProps {
  register: UseFormRegister<AtaConteudoFormData>
  errors: FieldErrors<AtaConteudoFormData>
}

export function PresidenciaSection({ register, errors }: SectionProps) {
  return (
...
export function MusicaSection({ register, errors }: SectionProps) {
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
...
export function AnunciosSection({ register, errors, control }: SectionProps & { control: Control<AtaConteudoFormData> }) {
  const { fields, append, remove } = useFieldArray({ control, name: 'anuncios' })
  ...
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button
          type="button"
          onClick={() => append({ id: generateId(), data: '', descricao: '' })}
          className="inline-flex items-center gap-1 text-sm font-medium text-brand-600 hover:text-brand-700"
        >
          <Plus className="h-3.5 w-3.5" /> Adicionar
        </button>
      </div>
      {fields.length === 0 && <p className="text-sm text-gray-400 dark:text-slate-500">Nenhum anúncio.</p>}
      <div className="space-y-3">
        {fields.map((field, index) => (
          <div key={field.id} className="flex items-start gap-3">
            <div className="w-28 shrink-0">
              <input {...register(`anuncios.${index}.data`)} placeholder="dd/mm/aaaa" className={inputClass} />
              {errors.anuncios?.[index]?.data && <p className={errorClass}>{errors.anuncios[index]?.data?.message}</p>}
            </div>
            <div className="flex-1">
              <input {...register(`anuncios.${index}.descricao`)} placeholder="Descrição do anúncio" className={inputClass} />
...
export function SacerdocioSection({ register, errors, control, desobrigacoesText, setDesobrigacoesText }: SectionProps & { 
  control: Control<AtaConteudoFormData>
  desobrigacoesText: string
  setDesobrigacoesText: (val: string) => void
}) {
  const { fields, append, remove } = useFieldArray({ control, name: 'apoios' })
    ...
    <section className="rounded-xl border border-gray-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
      ...
      <div className="mb-4">
        <div className="mb-2 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={() => append({ id: generateId(), cargo: '', nome_membro: '' })}
            className="inline-flex items-center gap-1 text-sm font-medium text-brand-600 hover:text-brand-700"
          >
            <Plus className="h-3.5 w-3.5" /> Adicionar
          </button>
        </div>
        {fields.length === 0 && <p className="mb-2 text-sm text-gray-400 dark:text-slate-500">Nenhum apoio.</p>}
        <div className="space-y-3">
          {fields.map((field, index) => (
            <div key={field.id} className="flex items-start gap-3">
              <div className="flex-1">
                <input {...register(`apoios.${index}.cargo`)} placeholder="Cargo" className={inputClass} />
                {errors.apoios?.[index]?.cargo && <p className={errorClass}>{errors.apoios[index]?.cargo?.message}</p>}
              </div>
              <div className="flex-[2]">
                <input {...register(`apoios.${index}.nome_membro`)} placeholder="Nome do membro" className={inputClass} />
                {errors.apoios?.[index]?.nome_membro && <p className={errorClass}>{errors.apoios[index]?.nome_membro?.message}</p>}
              </div>
              <button type="button" onClick={() => remove(index)} className="mt-2 shrink-0 text-gray-400 hover:text-red-600 dark:text-slate-500">
                <Trash2 className="h-4 w-4" />
              </button>
            </div>
          ))}
...
export function SacramentoSection({ register }: Pick<SectionProps, 'register'>) {
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
...
export function ProgramaSection({ register, errors, control }: SectionProps & { control: Control<AtaConteudoFormData> }) {
  const { fields, append, remove } = useFieldArray({ control, name: 'discursantes' })
    ...
    <section className="rounded-xl border border-gray-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
      <div className="mb-4 flex items-center justify-between">
        ...
        {fields.length < 3 && (
          <button
            type="button"
            onClick={() => append({ id: generateId(), ordem: (fields.length + 1) as 1 | 2 | 3, nome: '', tema: '' })}
            className="inline-flex items-center gap-1 text-sm font-medium text-brand-600 hover:text-brand-700"
          >
            <Plus className="h-3.5 w-3.5" /> Adicionar
          </button>
      ...
      {errors.discursantes && !Array.isArray(errors.discursantes) && <p className="mb-2 text-sm text-red-600">{errors.discursantes.message}</p>}
      <div className="space-y-3">
        {fields.map((field, index) => (
          <div key={field.id} className="flex items-start gap-3">
            <span className="mt-2 text-sm font-medium text-gray-400 dark:text-slate-500">{index + 1}.</span>
            <div className="flex-1">
              <input {...register(`discursantes.${index}.nome`)} placeholder="Nome do discursante" className={inputClass} />
              {errors.discursantes?.[index]?.nome && <p className={errorClass}>{errors.discursantes[index]?.nome?.message}</p>}
            </div>
            <div className="flex-[2]">
              <input {...register(`discursantes.${index}.tema`)} placeholder="Tema (opcional)" className={inputClass} />
            </div>
            <button type="button" onClick={() => remove(index)} className="mt-2 shrink-0 text-gray-400 hover:text-red-600 dark:text-slate-500">
              <Trash2 className="h-4 w-4" />
            </button>
...
export function EncerramentoSection({ register, errors }: SectionProps) {
  return (
    <section className="rounded-xl border border-gray-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
